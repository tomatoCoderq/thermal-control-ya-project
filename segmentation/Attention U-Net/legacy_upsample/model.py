"""Attention U-Net — legacy upsample decoder (pre-MPS fix).

Use this module to resume checkpoints from ``legacy_upsample/checkpoints/``.
New training should use ``../model.py`` (ConvTranspose2d decoder).
"""
...
_SEG = Path(__file__).resolve().parents[2]

...
def build_temporal_loaders(
    ...
    *,
    ...
    """Train/test loaders for ConvLSTM.

    backend:
      - ``irt`` — IRTDataset + yaml (default, needs build_irt_cache)
      - ``termo`` — unified TermoDataset from datasets/
    """
    if backend == "termo":
        from common.termo_data import DEFAULT_TERMO_ROOT, build_termo_temporal_loaders

        root = termo_root or DEFAULT_TERMO_ROOT
        bs = batch_size if batch_size is not None else 2
        nw = num_workers if num_workers is not None else 0
        return build_termo_temporal_loaders(
            root_dir=root,
    ...
    return _build_temporal_loaders_irt(
        cfg_path,
        size=size,
        test_every=test_every,
...
def _build_temporal_loaders_irt(
    cfg_path: str | Path,
    *,
    size: int = INPUT_SIZE,
    test_every: int = 4,

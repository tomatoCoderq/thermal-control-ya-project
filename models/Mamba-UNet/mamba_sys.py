"""VSSM / Mamba-UNet backbone (adapted from Mamba-UNet / VMamba).

Pure-PyTorch selective scan fallback — works on CPU/MPS without mamba_ssm CUDA kernels.
Defaults tuned for thermal IRT: in_chans=6, num_classes=1, 256×256 (patch_size=4).
"""
...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    """Reference selective scan (no CUDA). u/delta: (B, D, L); A: (D, N); B/C: (B, N, L) or (B, G, N, L)."""
    dtype_in = u.dtype
    u = u.float()
    delta = delta.float()
    ...
    if is_variable_B and B.dim() == 4:
        G = B.shape[1]
        H = dim // G
        B = B.unsqueeze(2).expand(-1, -1, H, -1, -1).reshape(batch, dim, dstate, B.shape[-1])
    if is_variable_C and C.dim() == 4:
        G = C.shape[1]
        H = dim // G
        C = C.unsqueeze(2).expand(-1, -1, H, -1, -1).reshape(batch, dim, dstate, C.shape[-1])
...
# Prefer CUDA kernels when available; otherwise pure PyTorch ref.
try:
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        B, H, W, C = x.shape
        p = self.dim_scale
        x = x.view(B, H, W, p, p, C // (p * p))
        x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H * p, W * p, C // (p * p))
        x = self.norm(x)
        return x
...
class SS2D(nn.Module):
    ...
    def A_log_init(d_state, d_inner, copies=1, device=None, merge=True):
        ...
        if copies > 1:
            A_log = A_log.unsqueeze(0).expand(copies, -1, -1).contiguous()
            if merge:
    ...
    def D_init(d_inner, copies=1, device=None, merge=True):
        ...
        if copies > 1:
            D = D.unsqueeze(0).expand(copies, -1).contiguous()
            if merge:
                D = D.flatten(0, 1)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=6, num_classes=1, depths=[2, 2, 2, 2], 

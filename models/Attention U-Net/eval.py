"""Eval Attention U-Net on 256×256 crops."""
...
ROOT = HERE.parents[1]
UNET = SEG / "U-Net"
# insert order: last wins → HERE first on path
...
from model import AttentionUNet, SoftDiceLoss

CKPT_BEST = HERE / "model_attunet_tsr_best.tar"
...
if __name__ == "__main__":
    ...
    model = AttentionUNet(img_ch=img.shape[0], output_ch=1)
    if CKPT_BEST.exists():
    ...
    print(f"in_ch={img.shape[0]} size={tuple(img.shape[-2:])} Dice={d:.4f} IoU={iou:.4f} loss={loss:.4f}")

"""Eval U-Net: yaml → irt_data → crop 256."""
...
CKPT_BEST = HERE / "model_unet_tsr_best.tar"


def load_weights(model: torch.nn.Module, path: Path) -> None:
    sd = torch.load(path, map_location="cpu", weights_only=True)
    # torch.compile wraps modules → keys like _orig_mod.enc_block1...
...
if __name__ == "__main__":
    _, ds, _ = make_loader(HERE / "dataset_tsr.yaml", train=False, size=256)

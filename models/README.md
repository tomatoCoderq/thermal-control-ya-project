...
```bash
python scripts/build_irt_cache.py --yaml models/U-Net/dataset_tsr.yaml --overwrite
```
...
Yaml по умолчанию: `models/U-Net/dataset_tsr.yaml`  
...
python scripts/build_irt_cache.py --yaml models/U-Net/dataset_pca.yaml
...
| Модель | Файлы |
|--------|--------|
| Attention U-Net | `models/Attention U-Net/model_attunet_{tsr\|fourier}_best.tar` — лучший по **test IoU** |
| | `…_last.tar` — последняя эпоха |
| Mamba-UNet | `models/Mamba-UNet/model_mamba_{tsr\|fourier}_best.tar` / `_last.tar` |
| U-Net | `models/U-Net/model_unet_{tsr\|fourier}_best.tar` / `_last.tar` |
...
- `models/Attention U-Net/runs/tsr/history.json`
- `models/Mamba-UNet/runs/fourier/metrics.csv`
...
```bash
cd /Users/user/Education/CVYandexCamp/thermal-control-ya-project/models
...
2. `python scripts/build_irt_cache.py --yaml models/U-Net/dataset_tsr.yaml`
3. `export NO_ALBUMENTATIONS_UPDATE=1`
4. `cd models/Attention\ U-Net && python train.py --yaml ../U-Net/dataset_tsr.yaml --variants tsr --epochs 50 --device mps`



source /Users/user/Education/CVYandexCamp/venv/bin/activate && cd /Users/user/Education/CVYandexCamp/thermal-control-ya-project && MPLBACKEND=Agg PYTHONPATH=. python models/common/plot_history.py "models/Attention U-Net/runs/tsr" --save "models/Attention U-Net/runs/tsr/curves.png" --no-show --title "Attention U-Net TSR" && python3 - <<'PY'
import json
from pathlib import Path
h=json.loads(Path('models/Attention U-Net/runs/tsr/history.json').read_text())
...
PY
ls -lh "models/Attention U-Net/runs/tsr/curves.png"

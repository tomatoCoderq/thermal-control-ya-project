...
def feature_signature(settings: dict) -> str:
...
def save_feature_cache(
    ...
    *,
    ...
    np.save(output, features.astype(np.float32, copy=False))
    frequencies = bins.astype(float) * sampling_hz / (
        settings["n_frames"] - start_frame
    )
    metadata = {
        ...
        "signature": feature_signature(settings),
    }
...
def cache_matches(output: str | Path, settings: dict) -> bool:
    output = Path(output)
    ...
    try:
        ...
        return metadata.get("signature") == feature_signature(settings)

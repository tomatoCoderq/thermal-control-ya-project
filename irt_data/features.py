...
class TSRCoeffsFeatureExtractor:
    """TSR coefficient maps: poly_degree+1 channels (deg=5 → 6)."""
    ...
    def __call__(self, video: np.ndarray) -> np.ndarray:
        v = _preprocess(video, self.frame_step, self.max_frames, self.thermal_diff)
        T, H, W = v.shape
        peak = int(np.argmax(v.reshape(T, -1).mean(1)))
        cool = v[peak:]
        if cool.shape[0] < self.poly_degree + 2:
            cool = v
        base = v[: max(1, peak // 4)].mean(axis=0) if peak > 0 else v[0]
        dT = np.clip(cool - base[None], 1e-3, None)
        tc = np.arange(1, dT.shape[0] + 1, dtype=np.float64)
        logt = np.log(tc)
        # polyfit expects (T, N); coef shape (deg+1, N) highest power first
        coef = np.polyfit(logt, np.log(dT).reshape(dT.shape[0], -1), self.poly_degree)
        return coef.T.reshape(H, W, self.poly_degree + 1).astype(np.float32)
...
FEATURE_REGISTRY: dict[str, type] = {
    ...
    "tsr_coeffs": TSRCoeffsFeatureExtractor,
